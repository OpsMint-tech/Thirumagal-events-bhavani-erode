export const translations = {
  en: {
    nav: {
      home: "Home",
      services: "Services",
      whyUs: "Why Us",
      gallery: "Gallery",
      about: "About",
      reviews: "Reviews",
      faq: "FAQ",
      bookNow: "Book Now"
    },
    hero: {
      label: "Erode's Trusted Event Planners Since 1991",
      titleStart: "We Turn Every Moment Into a ",
      titleGold: "Timeless Memory",
      titleEnd: "",
      subtext: "Premium wedding, reception & corporate event decoration across Bhavani, Erode, Komarapalayam & Coimbatore",
      ctaCall: "Call Now",
      ctaWhatsapp: "WhatsApp Us",
      ctaGallery: "View Our Work"
    },
    about: {
      subtitle: "Our Story & Legacy",
      title: "Crafting Timeless Celebrations Since 1991",
      desc1: "With over three decades of excellence, Thirumagal Events has evolved from a local decorator to Erode district's most trusted name in premium event management. Our journey began with a simple mission: to infuse every celebration with unparalleled elegance and traditional soul.",
      desc2: "We don't just decorate spaces; we curate experiences. Whether it's a grand celestial wedding, a sophisticated corporate gala, or an intimate family gathering, our team of seasoned artisans and planners ensure that every detail reflects your unique vision and our legacy of perfection.",
      btn: "Explore Our Journey",
      stats: {
        years: "35+ Years",
        events: "5000+ Events",
        happy: "Happy Clients"
      }
    },
    trust: {
      experience: "35+ Years of Trust",
      events: "500+ Events Decorated",
      locations: "Serving Erode & Coimbatore",
      delivery: "On-Time Delivery Guaranteed"
    },
    services: {
      label: "Our Decoration Services",
      title: "Best in Bhavani & Erode District",
      items: {
        wedding: { title: "Wedding Decoration", desc: "Elegant and grand stage setups for your special day." },
        reception: { title: "Reception Stage", desc: "Modern and floral stage designs to wow your guests." },
        birthday: { title: "Birthday Decoration", desc: "Creative and colorful themes for all ages." },
        engagement: { title: "Engagement Ceremony", desc: "Traditional and contemporary decor for engagements." },
        corporate: { title: "Corporate Events", desc: "Professional branding and stage setups for businesses." },
        baby: { title: "Baby Shower", desc: "Beautiful and warm settings for welcoming new life." },
        house: { title: "Housewarming", desc: "Traditional floral arrangements for your new home." },
        naming: { title: "Naming Ceremony", desc: "Auspicious decor for your child's special name day." }
      }
    },
    whyUs: {
      label: "Why Choose Us",
      title: "Why Families in Bhavani & Erode Trust Us Since 1991",
      badge: "Years of Legacy",
      points: {
        p1: { title: "35+ Years Experience", desc: "Since 1991, we have been the benchmark for quality and trust in the event decoration industry." },
        p2: { title: "500+ Events", desc: "Successfully delivered thousands of weddings, receptions, and corporate events with perfection." },
        p3: { title: "Serving All Erode District", desc: "From Bhavani to Coimbatore, we bring our premium decoration services to your doorstep." },
        p4: { title: "Fully Customised Packages", desc: "Tailored designs that fit your vision and budget without compromising on elegance." }
      }
    },
    gallery: {
      label: "Our Work Portfolio",
      title: "Our Work Across Bhavani, Erode & Coimbatore",
      hover: "View Gallery"
    },
    testimonials: {
      label: "Testimonials",
      title: "What Our Clients Say",
      list: {
        r1: { name: "Ananya Suresh", loc: "Bhavani", event: "Wedding Decoration", text: "Absolutely stunning work! They turned our wedding hall into a dream landscape. The floral arrangements were fresh and exactly as we imagined." },
        r2: { name: "Meena Krishnan", loc: "Erode", event: "Reception Stage", text: "The best decorators in Erode district. Their 35 years of experience really shows in their professionalism and attention to detail. Highly recommended!" },
        r3: { name: "Arjun Kumar", loc: "Coimbatore", event: "Corporate Events", text: "We hired them for our company's annual gala. The stage setup and branding backdrop were top-notch. On-time delivery and impeccable service." }
      }
    },
    faq: {
      label: "Common Questions",
      title: "Frequently Asked Questions",
      list: {
        q1: { q: "Do you offer wedding decoration in Bhavani?", a: "Yes, we serve Bhavani, Komarapalayam, Anthiyur, Gobichettipalayam and all of Erode District. We are conveniently located in Bhavani to serve the local community since 1991." },
        q2: { q: "What is the cost of wedding decoration in Erode?", a: "Our pricing is fully customised to your specific requirements, venue size, and budget. We offer premium decoration for every scale. Call +91 98429 30758 for a free consultations and a detailed quote." },
        q3: { q: "Do you handle corporate events?", a: "Yes, we handle corporate events across Erode and Coimbatore. Our services include professional stage setup, branding backdrops, floral arrangements, and complete venue transformation for seminars, galas, and product launches." },
        q4: { q: "How many years of experience do you have?", a: "We have over 35+ years of experience in the event decoration industry. Established in 1991, we have successfully delivered thousands of weddings, receptions, and corporate events with a legacy of trust." }
      }
    },
    contact: {
      label: "Get in Touch",
      title: "Book Your Decoration – Bhavani, Erode & Beyond",
      form: {
        name: "Name",
        namePlh: "Your Name",
        phone: "Phone",
        phonePlh: "Your Phone Number",
        eventType: "Event Type",
        selectEvent: "Select Event",
        eventDate: "Event Date",
        location: "Location",
        locPlh: "Event Location (e.g. Bhavani)",
        message: "Message",
        msgPlh: "Tell us about your requirements...",
        submit: "Submit Enquiry"
      },
      info: {
        call: "Call Us",
        whatsapp: "WhatsApp",
        visit: "Visit Our Office",
        chat: "Chat on WhatsApp",
        getDirections: "Get Directions",
        successTitle: "Thank You!",
        successMsg: "Your enquiry has been sent. Opening WhatsApp for chat..."
      }
    },
    footer: {
      tagline: "Since 1991 \u2014 Erode's most trusted event decorators for over 35 years.",
      contactTitle: "Contact Details",
      linksTitle: "Quick Links",
      rights: "All rights reserved.",
      powered: "Designed with ❤️ by "
    },
    eventTypes: {
      subtitle: "Our Speciality",
      title: "Weddings, Receptions & More",
      desc: "We bring artistry and elegance to every event — from intimate engagements to grand wedding ceremonies.",
      feature1: "Grand Wedding Stage Setups",
      feature2: "Premium Floral Decorations",
      feature3: "Professional Event Management",
      btn: "Book Your Event",
      servicesHeader: "Our Services",
      servicesTitle: "Premium Decoration for Every Occasion"
    },
    carousel: {
      subtitle: "Our Packages",
      title: "Decoration Services & Pricing",
      desc: "Transparent pricing for every event type. All packages are fully customisable.",
      unit: "Per Event",
      bookBtn: "Book Now",
      items: {
        birthday: { title: "Birthday Decoration", desc: "Creative balloon & floral setups for birthday celebrations of all ages." },
        wedding: { title: "Wedding Decoration", desc: "Grand stage setups and floral arrangements for your most special day." },
        stage: { title: "Stage Decoration", desc: "Premium stage backdrops with lighting and flower arrangements." },
        entrance: { title: "Entrance Decoration", desc: "Beautiful entrance arches and floral corridors to welcome your guests." },
        engagement: { title: "Engagement Decoration", desc: "Elegant and romantic setups for your engagement ceremony." },
        balloon: { title: "Balloon Decoration", desc: "Colourful balloon sculptures, arches, and photo-booth setups." }
      }
    },
    pricing: {
      subtitle: "Pricing Plans",
      title: "Choose the Right Package",
      starting: "Starting from",
      label: "What's Included:",
      btn: "Book This Plan",
      p1: { title: "Full Planning", desc: "Complete end-to-end event planning and decoration service." },
      p2: { title: "Partial Planning", desc: "We handle key parts of your event while you manage the rest." },
      p3: { title: "Day-Of Service", desc: "We coordinate everything on your event day for a stress-free experience." }
    },
    cta: {
      title: "Ready to Make Your Event Unforgettable?",
      btn: "Book a Free Consultation"
    }
  },
  ta: {
    nav: {
      home: "முகப்பு",
      services: "சேவைகள்",
      whyUs: "ஏன் நாம்",
      gallery: "புகைப்படங்கள்",
      about: "எங்களைப் பற்றி",
      reviews: "கருத்துக்கள்",
      faq: "கேள்விகள்",
      bookNow: "முன்பதிவு"
    },
    hero: {
      label: "1991 முதல் ஈரோட்டின் நம்பகமான நிகழ்ச்சி ஏற்பாட்டாளர்கள்",
      titleStart: "ஒவ்வொரு தருணத்தையும் ஒரு ",
      titleGold: "காலமற்ற நினைவாக",
      titleEnd: " மாற்றுகிறோம்",
      subtext: "பவானி, ஈரோடு, குமாரபாளையம் மற்றும் கோயம்புத்தூர் முழுவதும் பிரீமியம் திருமணம் மற்றும் கார்ப்பரேட் அலங்காரம்",
      ctaCall: "அழைக்க",
      ctaWhatsapp: "வாட்ஸ்அப்",
      ctaGallery: "எங்கள் வேலைகளைப் பாருங்கள்"
    },
    about: {
      subtitle: "எங்கள் கதை மற்றும் பாரம்பரியம்",
      title: "1991 முதல் காலமற்ற கொண்டாட்டங்களை உருவாக்குதல்",
      desc1: "மூன்று தசாப்தங்களுக்கும் மேலான சிறந்து விளங்கும் திரு மகள் ஈவென்ட்ஸ், ஒரு உள்ளூர் அலங்கார நிறுவனத்திலிருந்து ஈரோடு மாவட்டத்தின் மிகவும் நம்பகமான பிரீமியம் நிகழ்வு மேலாண்மை பெயராக வளர்ந்துள்ளது. எங்களின் பயணம் ஒரு எளிய நோக்கத்துடன் தொடங்கியது: ஒவ்வொரு கொண்டாட்டத்திலும் ஈடு இணையற்ற நேர்த்தியையும் பாரம்பரிய ஆன்மாவையும் புகுத்துவது.",
      desc2: "நாங்கள் வெறும் இடங்களை மட்டும் அலங்கரிப்பதில்லை; நாங்கள் அனுபவங்களை உருவாக்குகிறோம். அது ஒரு பிரம்மாண்டமான திருமணமானாலும், ஒரு அதிநவீன கார்ப்பரேட் விழாவானாலும் அல்லது ஒரு நெருக்கமான குடும்பக் கூடலானாலும், எங்களின் அனுபவம் வாய்ந்த கலைஞர்கள் மற்றும் திட்டமிடுபவர்கள் ஒவ்வொரு விவரமும் உங்களின் தனித்துவமான பார்வையையும் எங்களின் சிறப்பான பாரம்பரியத்தையும் பிரதிபலிப்பதை உறுதி செய்கிறார்கள்.",
      btn: "எங்கள் பயணத்தை ஆராயுங்கள்",
      stats: {
        years: "35+ ஆண்டுகள்",
        events: "5000+ நிகழ்ச்சிகள்",
        happy: "மகிழ்ச்சியான வாடிக்கையாளர்கள்"
      }
    },
    trust: {
      experience: "35+ ஆண்டுகால நம்பிக்கை",
      events: "500+ நிகழ்ச்சிகள் அலங்கரிக்கப்பட்டுள்ளன",
      locations: "ஈரோடு & கோயம்புத்தூர் சேவை",
      delivery: "சரியான நேரத்திற்கு சேவை உறுதி"
    },
    services: {
      label: "எங்கள் அலங்காரச் சேவைகள்",
      title: "பவானி மற்றும் ஈரோடு மாவட்டத்தில் சிறந்தது",
      items: {
        wedding: { title: "திருமண அலங்காரம்", desc: "உங்கள் சிறப்பு தினத்திற்கான நேர்த்தியான மற்றும் பிரமாண்டமான மேடை அமைப்புகள்." },
        reception: { title: "வரவேற்பு மேடை", desc: "உங்கள் விருந்தினர்களைக் கவர நவீன மற்றும் மலர் மேடை வடிவமைப்புகள்." },
        birthday: { title: "பிறந்தநாள் அலங்காரம்", desc: "அனைத்து வயதினருக்கும் ஆக்கப்பூர்வமான மற்றும் வண்ணமயமான தீம்கள்." },
        engagement: { title: "நிச்சயதார்த்த விழா", desc: "நிச்சயதார்த்தங்களுக்கான பாரம்பரிய மற்றும் சமகால அலங்காரம்." },
        corporate: { title: "கார்ப்பரேட் நிகழ்வுகள்", desc: "வணிகங்களுக்கான தொழில்முறை பிராண்டிங் மற்றும் மேடை அமைப்புகள்." },
        baby: { title: "வளைகாப்பு விழா", desc: "புதிய உயிர் வரவேற்பிற்கான அழகான மற்றும் சூடான அமைப்புகள்." },
        house: { title: "புதுமனை புகுவிழா", desc: "உங்கள் புதிய வீட்டிற்கான பாரம்பரிய மலர் ஏற்பாடுகள்." },
        naming: { title: "பெயர் சூட்டும் விழா", desc: "குழந்தையின் சிறப்பு பெயர் சூட்டு விழாவிற்கு மங்கலகரமான அலங்காரம்." }
      }
    },
    whyUs: {
      label: "ஏன் எங்களைத் தேர்ந்தெடுக்க வேண்டும்",
      title: "1991 முதல் பவானி மற்றும் ஈரோடு குடும்பங்கள் ஏன் எங்களை நம்புகின்றன",
      badge: "ஆண்டுகால பாரம்பரியம்",
      points: {
        p1: { title: "35+ ஆண்டுகால அனுபவம்", desc: "1991 முதல், நாங்கள் நிகழ்வு அலங்காரத் துறையில் தரம் மற்றும் நம்பிக்கையின் அடையாளமாக இருக்கிறோம்." },
        p2: { title: "500+ நிகழ்ச்சிகள்", desc: "ஆயிரக்கணக்கான திருமணங்கள், வரவேற்புகள் மற்றும் கார்ப்பரேட் நிகழ்வுகளை மிகச்சரியாக வழங்கியுள்ளோம்." },
        p3: { title: "ஈரோடு மாவட்டம் முழுவதும்", desc: "பவானி முதல் கோயம்புத்தூர் வரை, நாங்கள் உங்கள் வீட்டு வாசலுக்கே பிரீமியம் அலங்காரச் சேவைகளைக் கொண்டு வருகிறோம்." },
        p4: { title: "முழுமையாக தனிப்பயனாக்கப்பட்ட சேவைகள்", desc: "நேர்த்தியில் சமரசம் செய்யாமல் உங்களின் பார்வை மற்றும் பட்ஜெட்டுக்கு ஏற்ற வடிவமைப்புகள்." }
      }
    },
    gallery: {
      label: "எங்கள் பணித் தொகுப்பு",
      title: "பவானி, ஈரோடு & கோயம்புத்தூர் முழுவதும் எங்கள் பணிகள்",
      hover: "புகைப்படத்தைப் பார்க்க"
    },
    testimonials: {
      label: "வாடிக்கையாளர் கருத்துக்கள்",
      title: "எங்கள் வாடிக்கையாளர்கள் கூறுவது",
      list: {
        r1: { name: "அனன்யா சுரேஷ்", loc: "பவானி", event: "திருமண அலங்காரம்", text: "நிச்சயமாக பிரமிக்க வைக்கும் வேலை! அவர்கள் எங்கள் திருமண மண்டபத்தை ஒரு கனவு நிலப்பரப்பாக மாற்றினர். மலர் ஏற்பாடுகள் புத்துணர்ச்சியுடன் நாங்கள் கற்பனை செய்தது போலவே இருந்தன." },
        r2: { name: "மீனா கிருஷ்ணன்", loc: "ஈரோடு", event: "வரவேற்பு மேடை", text: "ஈரோடு மாவட்டத்தின் சிறந்த அலங்கார நிபுணர்கள். அவர்களின் 35 ஆண்டுகால அனுபவம் அவர்களின் தொழில்முறை மற்றும் நுணுக்கங்களில் தெளிவாகத் தெரிகிறது. மிகவும் பரிந்துரைக்கிறேன்!" },
        r3: { name: "அர்ஜுன் குமார்", loc: "கோயம்புத்தூர்", event: "கார்ப்பரேட் நிகழ்வுகள்", text: "எங்கள் நிறுவனத்தின் வருடாந்திர விழாவிற்காக அவர்களை நியமித்தோம். மேடை அமைப்பு மற்றும் பிராண்டிங் பின்னணி முதன்மையானதாக இருந்தது. சரியான நேரத்தில் டெலிவரி மற்றும் குறைபாடற்ற சேவை." }
      }
    },
    faq: {
      label: "பொதுவான கேள்விகள்",
      title: "அடிக்கடி கேட்கப்படும் கேள்விகள்",
      list: {
        q1: { q: "பவானியில் திருமண அலங்காரச் சேவை வழங்குகிறீர்களா?", a: "ஆம், பவானி, குமாரபாளையம், அந்தியூர், கோபிசெட்டிபாளையம் மற்றும் ஈரோடு மாவட்டம் முழுவதும் சேவை செய்கிறோம். 1991 முதல் உள்ளூர் மக்களுக்கு சேவை செய்ய பவானியில் வசதியாக இருக்கிறோம்." },
        q2: { q: "ஈரோட்டில் திருமண அலங்காரத்திற்கு எவ்வளவு செலவாகும்?", a: "எங்கள் விலை உங்கள் குறிப்பிட்ட தேவைகள், இடத்தின் அளவு மற்றும் பட்ஜெட்டுக்கு ஏற்ப அமைக்கப்படுகிறது. எல்லா நிலைகளுக்கும் பிரீமியம் அலங்காரத்தை வழங்குகிறோம். இலவச ஆலோசனைக்கு +91 98429 30758 ஐ அழைக்கவும்." },
        q3: { q: "கார்ப்பரேட் நிகழ்வுகளைக் கையாளுவீர்களா?", a: "ஆம், ஈரோடு மற்றும் கோயம்புத்தூர் முழுவதும் கார்ப்பரேட் நிகழ்வுகளைக் கையாளுகிறோம். தொழில்முறை மேடை அமைப்பு, பிராண்டிங் பின்னணிகள் மற்றும் மலர் ஏற்பாடுகள் ஆகியவையும் இதில் அடங்கும்." },
        q4: { q: "உங்களுக்கு எத்தனை வருட அனுபவம் உள்ளது?", a: "நிகழ்வு அலங்காரத் துறையில் 35+ ஆண்டுகளுக்கும் மேலான அனுபவம் எங்களிடம் உள்ளது. 1991-ல் நிறுவப்பட்ட நாங்கள், ஆயிரக்கணக்கான நிகழ்ச்சிகளை நம்பிக்கையுடனும் பாரம்பரியத்துடனும் வழங்கியுள்ளோம்." }
      }
    },
    contact: {
      label: "தொடர்பு கொள்ள",
      title: "உங்கள் அலங்காரத்தை முன்பதிவு செய்யுங்கள் – பவானி, ஈரோடு & பிற இடங்கள்",
      form: {
        name: "பெயர்",
        namePlh: "உங்கள் பெயர்",
        phone: "தொலைபேசி",
        phonePlh: "உங்கள் தொலைபேசி எண்",
        eventType: "நிகழ்வு வகை",
        selectEvent: "நிகழ்வைத் தேர்ந்தெடுக்கவும்",
        eventDate: "நிகழ்வு தேதி",
        location: "இடம்",
        locPlh: "நிகழ்வு இடம் (உதாரணமாக பவானி)",
        message: "செய்தி",
        msgPlh: "உங்கள் தேவைகளைப் பற்றி சொல்லுங்கள்...",
        submit: "விசாரணையை அனுப்பவும்"
      },
      info: {
        call: "அழைக்க",
        whatsapp: "வாட்ஸ்அப்",
        visit: "எங்கள் அலுவலகம்",
        chat: "வாட்ஸ்அப்பில் சாட் செய்ய",
        getDirections: "திசை அறிக்கை",
        successTitle: "மிக்க நன்றி!",
        successMsg: "உங்கள் விசாரணை அனுப்பப்பட்டது. வாட்ஸ்அப் பக்கம் திறக்கப்படும்..."
      }
    },
    footer: {
      tagline: "1991 முதல் \u2014 35 ஆண்டுகளுக்கும் மேலாக ஈரோட்டின் மிகவும் நம்பகமான நிகழ்வு அலங்கார நிபுணர்கள்.",
      contactTitle: "தொடர்பு விவரங்கள்",
      linksTitle: "விரைவான இணைப்புகள்",
      rights: "அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டவை.",
      powered: "❤️ உடன் வடிவமைத்தவர்கள் "
    },
    eventTypes: {
      subtitle: "எங்கள் சிறப்பு",
      title: "திருமணங்கள், வரவேற்புகள் & மேலும்",
      desc: "நெருங்கிய நிச்சயதார்த்தங்கள் முதல் பிரமாண்டமான திருமண விழாக்கள் வரை ஒவ்வொரு நிகழ்வுக்கும் அழகியல் மற்றும் நேர்த்தி தருகிறோம்.",
      feature1: "பிரமாண்டமான திருமண மேடை அமைப்புகள்",
      feature2: "பிரீமியம் மலர் அலங்காரங்கள்",
      feature3: "தொழில்முறை நிகழ்வு மேலாண்மை",
      btn: "நிகழ்வை முன்பதிவு செய்யுங்கள்",
      servicesHeader: "எங்கள் சேவைகள்",
      servicesTitle: "ஒவ்வொரு சந்தர்ப்பத்திற்கும் பிரீமியம் அலங்காரம்"
    },
    carousel: {
      subtitle: "எங்கள் திட்டங்கள்",
      title: "அலங்காரச் சேவைகள் & விலை",
      desc: "ஒவ்வொரு நிகழ்வு வகைக்கும் வெளிப்படையான விலை நிர்ணயம். அனைத்து திட்டங்களும் முழுமையாக தனிப்பயனாக்கலாம்.",
      unit: "நிகழ்வுக்கு",
      bookBtn: "முன்பதிவு செய்யுங்கள்",
      items: {
        birthday: { title: "பிறந்தநாள் அலங்காரம்", desc: "அனைத்து வயதினரின் பிறந்தநாள் கொண்டாட்டங்களுக்கு ஆக்கப்பூர்வமான பலூன் மற்றும் மலர் அமைப்புகள்." },
        wedding: { title: "திருமண அலங்காரம்", desc: "உங்கள் மிக சிறப்பான நாளுக்கான பிரமாண்டமான மேடை அமைப்புகள் மற்றும் மலர் ஏற்பாடுகள்." },
        stage: { title: "மேடை அலங்காரம்", desc: "விளக்குகள் மற்றும் மலர் ஏற்பாடுகளுடன் பிரீமியம் மேடை பின்னணிகள்." },
        entrance: { title: "நுழைவாயில் அலங்காரம்", desc: "உங்கள் விருந்தினர்களை வரவேற்க அழகான நுழைவாயில் வளைவுகள் மற்றும் மலர் சுரங்கங்கள்." },
        engagement: { title: "நிச்சயதார்த்த அலங்காரம்", desc: "உங்கள் நிச்சயதார்த்த விழாவிற்கு நேர்த்தியான மற்றும் ரோமாந்தமான அமைப்புகள்." },
        balloon: { title: "பலூன் அலங்காரம்", desc: "வண்ணமயமான பலூன் சிற்பங்கள், வளைவுகள் மற்றும் போட்டோ-பூத் அமைப்புகள்." }
      }
    },
    pricing: {
      subtitle: "விலை திட்டங்கள்",
      title: "சரியான திட்டத்தைத் தேர்ந்தெடுங்கள்",
      starting: "தொடங்கும் விலை",
      label: "என்ன சேர்க்கப்பட்டுள்ளது:",
      btn: "இந்த திட்டத்தை முன்பதிவு செய்யுங்கள்",
      p1: { title: "முழு திட்டமிடல்", desc: "முழுமையான நிகழ்வு திட்டமிடல் மற்றும் அலங்காரச் சேவை." },
      p2: { title: "பகுதி திட்டமிடல்", desc: "நீங்கள் மீதியை நிர்வகிக்கும் போது நாங்கள் முக்கிய பகுதிகளை கையாளுகிறோம்." },
      p3: { title: "நிகழ்வு நாள் சேவை", desc: "அழுத்தமற்ற அனுபவத்திற்காக உங்கள் நிகழ்வு நாளில் எல்லாவற்றையும் ஒருங்கிணைக்கிறோம்." }
    },
    cta: {
      title: "உங்கள் நிகழ்வை மறக்கமுடியாததாக மாற்ற தயாரா?",
      btn: "இலவச ஆலோசனையை முன்பதிவு செய்யுங்கள்"
    }
  }
};
